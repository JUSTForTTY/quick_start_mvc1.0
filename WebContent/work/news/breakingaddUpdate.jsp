...
<script type="text/javascript">
		...
		//	alert("保存操作");
			...
			map['Date']=$("#Date").datebox('getValue');
		//	alert("Date"+$("#Date").datebox('getValue'));
			map['providerId']=$.trim($('#providerId').val());
	    ...
	    //    map['id']=$.trim($('#id').val());	
	     //	alert(image);
	     	//map['image']=$.trim($('#image').val());	
				map['id']=id;
				//alert("去后台"+id);
				newsAction.getsaveOrUpdate(map,{
	            	async : false,
					callback : function(data) {
						if (data.success) {
			...
			<tr>
				<td>		
					<div class="lableLine350">
						<div class="lableWidth120">大事纪标题</div>
					...
					<div class="lableLine350" id="nihao">
						<div class="lableWidth120" >会员单位编号</div>
						<input name="providerId" id="providerId" style="width: 350px;"
							class="easyui-validatebox"
							data-options="required:false,validType:'length[1,100]'" />

					</div>
			<div class="lableLine350"> 
				<div class="lableWidth120">大事件时间</div>
				<input id="Date" name="Date"  value=""  style=""
...
<!-- 						<div class="lableWidth120">公司简介</div> -->
