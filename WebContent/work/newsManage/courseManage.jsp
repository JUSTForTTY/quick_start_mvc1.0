...
<script type="text/javascript">
	...
	function editBtnClick(nid,caid,obj) {
		...
		easyui_openTopWindow("课程体系管理",828,640,url,function(returnValue)
		{
			$('#dg').datagrid('reload');
		}); 
	}
...
//查询已删除的数据
	...
	function searchDate(caid,obj){
		var url="<%=basePath%>work/newsManage/courseRecoveryManage.jsp?caid="+caid; 
		easyui_openTopWindow("课程体系管理",1500,640,url,function(returnValue)
				{
					$('#dg').datagrid('reload');
				}); 
			}
		...
		<table id="dg" class="easyui-datagrid" title=" "
			data-options="
						fitColumns:false,
	                     ...
	                     url:newsManageAction.getCourseList">
			<thead>
				<tr>
                   ...
                   <th data-options="field:'onhome',width:250,editor:{type:'validatebox',options:{required:true,validType:'length[1,11]'}},align:'center',halign:'center'">首页显示</th> 
                    <th data-options="field:'briefintro',width:250,editor:{type:'validatebox',options:{required:true,validType:'length[1,200]'}},align:'center',halign:'center'">简介</th>
				  <th data-options="field:'status',width:250,editor:{type:'validatebox',options:{required:true,validType:'length[1,200]'}},align:'center',halign:'center'">是否删除</th>
...
<!-- 		 <a id="delBtn" href="javascript:void(0)" class="easyui-linkbutton" style="margin-left: 10px;" -->
<%-- 				data-options="iconCls:'icon-search',plain:true" onclick="searchDate('<%=request.getParameter("caid") %>',this)">查询已删除数据</a>  --%>
		</div>
	</div>
</body>
</html>
