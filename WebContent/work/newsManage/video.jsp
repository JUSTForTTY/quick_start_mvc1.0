...
<script type="text/javascript">
	...
	function editBtnClick(nid,caid,obj) {
		...
		var url="<%=basePath%>work/newsManage/subVideo.jsp?nid="+nid+"&&caid="+caid;
		
		easyui_openTopWindow("相关视频管理",807,306,url,function(returnValue)
		{
			$('#dg').datagrid('reload');
		}); 
	}
	...
	//	alert(caid);
		...
		var url="<%=basePath%>work/newsManage/subVideo.jsp?caid="+caid;
		
		easyui_openTopWindow("视频新闻管理",807,306,url,function(returnValue)
		{
			$('#dg').datagrid('reload');
		}); 
	}
		...
		<table id="dg" class="easyui-datagrid" title="  "
			...
			<thead>
				<tr>
                    ...
                    <th data-options="field:'title',width:150,editor:{type:'validatebox',options:{required:true,validType:'length[1,20]'}},align:'center',halign:'center'">标题</th>
                    <th data-options="field:'englishTitle',width:300,editor:{type:'validatebox',options:{required:true,validType:'length[1,20]'}},align:'center',halign:'center'">英文标题</th>
                  <th data-options="field:'onhome',width:150,editor:{type:'validatebox',options:{required:true,validType:'length[1,20]'}},align:'center',halign:'center'">首页显示</th>
                   <th data-options="field:'url',width:250,editor:{type:'validatebox',options:{required:true,validType:'length[1,20]'}},align:'center',halign:'center'">链接</th>				
