...
(function($) {
	$.addFlex = function(t, p) {
		...
		p = $.extend({ // apply default properties
			...
			title : false,
			idProperty : 'id',
			pagestat : '从{from}到{to}条 , 共 {total} 条',
			pagetext : '当前页:',
			outof : '总页数:',
			findtext : 'Find',
		...
		var g = {
			...
			rePosDrag : function() {
				...
				$('thead tr:first th:visible', this.hDiv).each(function() {
					...
					if($.browser.msie && $.browser.version.substr(0,1) == "8") 
					    $('div:eq('+n+')',g.cDrag).css({'left':(cdpos-(1+n))+'px'}).show(); 
					else 
			...
			dragEnd : function() {
				if (this.colresize) {
					...
					$('tr', this.bDiv).each(function() {
						...
						/*$tdDiv.css('width', nw);*/
						//万利 2013.2.19 追加代码 拖拽的时候设置文本框的长度
						$tdDiv.css('width', nw).find("input:text,select").css('width', nw);
						g.addTitleToCell($tdDiv);
					});
						
					//万利 2013.1.24 动态改变模板列的宽度
					$(t).data("template").find('td div:eq(' + n + ')').css('width', nw).find("input[type='text'],select").css('width', nw);
					...
					if ($.cookies) {
						var name = p.colModel[n].name; // Store the widths
						// in the cookies
			...
			toggleCol : function(cid, visible) {
				...
				$(t).data("template").eq(0).each(function(){
					if (visible) {
						$('td:eq(' + n + ')', this).show();
					} else {
						$('td:eq(' + n + ')', this).hide();
			...
			addData1 : function(data) { // parse data
				if (p.dataType == 'json') {
					data = $.extend({
						rows : [],
						page : 0,
			...
			updateState : function(page) {
				//更新当前页面页码
				if(page){
					p.pages = page['totalPage'] ;
					p.page   = page['nowPage'] ;
					p.total    = page['totalRecords'];
				}
			...
			updaterp:function(rpO){
				if(!rpO) {
					rpO  = p.rpOptions ;
				}
				else
				{
					//如果不分页，将当前控件的每页数 更新为总的记录数
					p.rp = rpO[0];
				}
				if (p.useRp) {
					var opt = '', sel = '';
					for ( var nx = 0; nx < rpO.length; nx++) {
						if (p.rp == rpO[nx])
							sel = 'selected="selected"';
						else
							sel = '';
						opt += "<option value='" + rpO[nx] + "' " + sel
								+ " >" + rpO[nx] + "&nbsp;&nbsp;</option>";
					}
					$('.pDiv2', g.pDiv).find("[name=rp]")
							.html(opt);
			...
			populate : function(nowPage) { // get latest data
				if (this.loading) {
					return true;
				}
				if (p.onSubmit) {
				...
				//TODO 根据参数设置当前页  add by shant
				//alert("from flexigrid.js--populate: nowPage="+nowPage);
//				if(nowPage!=null){
//					p.newp = nowPage;
//				}
				
				//判断当前每页显示记录数，是否在页码的序列中，如果不在，则更新
				var _tmp = false;
				$(p.rpOptions).each(function(i,_data){
					if(_data== p.rp){
						_tmp = true;
						return false;
					}
				...
				// 加载服务器数据（分页）
				if (p.loadFunc) {
					p.loadFunc.call(this, param, p); //第一个参数，当前表格对象，第2个参数 分页使用的参数，第3个参数为当前表格对象的属性（设置值）
				}
				// $.ajax({
				// type: p.method,
				// url: p.url,
		...
		if (p.combobuttons && $(g.tDiv2)) {
			...
			for (i = 0; i < p.combobuttons.length; i++) {
				...
				if (!btn.separator) {
					...
					if (btn.bimage) // if bimage defined, use its string as
						// an image url for this buttons style
						// (RS)
						$(btnOpt)
								.css(
										'background',
		...
		return this.each(function() {
			if (!docloaded) {
				...
				$(document).ready(function() {
					...
					//for ie8
					if($.browser.msie && $.browser.version.substr(0,1) == "8"){
						if(!(document.body.scrollHeight-document.body.clientHeight>0) && p.autoHeight != 'false')
						{
							var bDiv = thisObj.parent();
							var offset = bDiv.offset();
							var hDiv = bDiv.prevAll(".hDiv");
							var pDiv = bDiv.nextAll(".pDiv");
							bDiv.height($(window).height() - offset.top - hDiv.height() - pDiv.height() - 5);
						}
					}// for chrome 					    
					else{
						if(document.body.scrollHeight-document.body.clientHeight>0 && p.autoHeight != 'false')
	...
	$.fn.flexReload = function(n) { // function to reload grid
		//alert("from flexigrid.js--fn.flexReload: nowPage="+nowPage);
		return this.each(function() {
			if (this.grid)
				this.grid.populate(n);
		});
	}; // end flexReload
